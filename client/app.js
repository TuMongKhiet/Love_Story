...
function syncTimelineEditorUI() {
  ...
  if (timelineEditorHint) {
    if (!loggedIn) {
      ...
      timelineEditorHint.textContent =
        "You are in edit mode. You can type, choose stickers, add boxes, and delete boxes.";
    } else {
      timelineEditorHint.textContent =
...
function prepareMusicPlaylist(forceShuffle = false) {
  ...
  if (!hasPreparedPlaylist || forceShuffle || !shuffledPlaylist.length) {
...
function loadCurrentTrack() {
  if (!bgMusic || !musicPlaylist.length) return;
  ...
  if (bgMusic.getAttribute("src") !== currentTrack) {
